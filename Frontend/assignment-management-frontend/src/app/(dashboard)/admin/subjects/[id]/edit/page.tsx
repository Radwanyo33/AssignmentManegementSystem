...
import { Subject, Class, User } from '@/types';
...
export default function EditSubject() {
    ...
    const loadData = async () => {
        ...
        setError(null);
        try {
            const [subjectData, classesData, usersData] = await Promise.all([
            ...
            setFormData({
                ...
                classId: subjectData.classId?.toString() || '',
                teacherId: subjectData.teacherId?.toString() || '',
            });
        } catch (error: any) {
            console.error('Failed to load data:', error);
            setError(error?.response?.data || 'Failed to load subject');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setError(error?.response?.data || error?.message || 'Failed to update subject');
        } finally {
            setSubmitting(false);
        }
    };
                ...
                <div className="max-w-3xl mx-auto py-6 sm:px-6 lg:px-8">
                    <div className="px-4 py-6 sm:px-0">
                        <div className="bg-red-50 border border-red-200 rounded-lg p-6 text-center">
                            <p className="text-red-600 font-medium">{error || 'Subject not found'}</p>
                            <button
                ...
                <div className="px-4 py-6 sm:px-0">
                    <div className="flex justify-between items-center mb-6">
                        <div>
                            ...
                            <p className="text-sm text-gray-500">Update subject: {subject.name}</p>
                        </div>
                        <button
                            onClick={() => router.push('/admin/subjects')}
